@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            {{-- <div>
                <p class="text-xs leading-5 text-gray-700 dark:text-gray-400">
                    {!! __('Showing') !!}
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-lg font-extrabold leading-5 text-white bg-gray-400 border border-gray-300 cursor-default dark:bg-gray-800 dark:border-gray-600">{{ $page }}</span>
                                    </span>
                                @else
